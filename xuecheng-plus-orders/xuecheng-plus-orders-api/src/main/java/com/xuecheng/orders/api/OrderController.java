...
public class OrderController {
    ...
    public PayRecordDto generatePayCode(@RequestBody  AddOrderDto addOrderDto){
        ...
        if(user == null){
            OnlieEducationException.cast("请登录后继续选课");
        }
    ...
    public void requestPay(String payNo, HttpServletResponse response) throws IOException {
        ...
        if (payRecord == null){
            OnlieEducationException.cast("请重新点击支付获取二维码");
        }
        //支付状态
        String status = payRecord.getStatus();
        if ("601002".equals(status)){
            //601002订单已支付
            OnlieEducationException.cast("订单已支付，请勿重复支付");
        }
        //构造SDK的客户端对象
        DefaultAlipayClient client = new DefaultAlipayClient(AlipayConfig.URL, APP_ID, APP_PRIVATE_KEY, AlipayConfig.FORMAT, AlipayConfig.CHARSET, ALIPAY_PUBLIC_KEY, AlipayConfig.SIGNTYPE);
        //创建API对应的request
        AlipayTradeWapPayRequest alipayRequest = new AlipayTradeWapPayRequest();

        //        alipayRequest.setReturnUrl("http://domain.com/CallBack/return_url.jsp");
        //NotifyUrl 支付宝将支付结果通知重定向到指定的地址，后续需要改成自己的地址
